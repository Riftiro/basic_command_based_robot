...
public class DriveSystem extends Subsystem implements PIDOutput {
	...
	private static double maxSpeed = 0.75;										//maxSpeed variable
    ...
    public void normalizedSpeed(double left, double right){
    	if (left > -1*RobotMap.LEFT_GO_MAX)
    		left = -1*RobotMap.LEFT_GO_MAX;	
    	
    	if (left < -1*RobotMap.LEFT_BACK_MAX)
    		left = -1*RobotMap.LEFT_BACK_MAX;
    	//
    	if (right > -1*RobotMap.RIGHT_GO_MAX)
    		right = -1*RobotMap.RIGHT_GO_MAX;
    	
    	if (right < -1*RobotMap.RIGHT_BACK_MAX)
    		right = -1*RobotMap.RIGHT_BACK_MAX;
    ...
    public void driveStraightReverse(double speed){
    	try {
             driveSystem.drive(speed, adx_Gyro.getAngle()*0.003);
    ...
    public void driveStraight(double speed){
        try {
            	driveSystem.drive(speed, -adx_Gyro.getAngle()*0.2);
        } catch( RuntimeException ex ) {
            DriverStation.reportError("Error communicating with drive system:  " + ex.getMessage(), true);
        }
